...
def step(Qnow):
    ...
    # s_t = [sx_t,sy_t,kx_t*100,ky_t*100]
    s_t = [sx_t,sy_t]
    s_t = np.array(s_t)
    k_t = np.array([kx_t,ky_t])
    return s_t, k_t


def output_data(episode_number, step, action, reward, state_t, s0, rt, k):
    name = "output.plt"
    ...
    if (not os.path.exists("test_results/" + name)):
        with open("test_results/" + name, "w") as csv_file:
            ...
            spam_writer.writerow([episode_number, step, action[0], action[1], reward, rt, state_t[0], state_t[1], k[0], k[1]])
            # spam_writer.writerow(['Episode', 'Step', 'Action1', 'Action2', 'Reward', 'rt', 'sx', 'sy'])
            # spam_writer.writerow([episode_number, step, action[0], action[1], reward, rt, state_t[0], state_t[1]])
...
for i in range(max_episode):
    ...
    for j in range(max_step):
        ...
        if (rt < r_threshold):
            STOP = True
        output_data(i+1, j+1, a, r, s_, s, rt, k)
        s = s_
        ep_r += r
